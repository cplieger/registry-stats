...
// Compile-time assertion: *healthMarker satisfies api.HealthSignal so
// internal/webapi handlers can consume it through the interface without
// reaching into the main package.
var _ api.HealthSignal = (*healthMarker)(nil)
...
// the api.HealthSignal interface so handlers in internal/webapi can
// render /api/health without reaching into a package global. Strict
// os.Stat: a degraded marker directory (read-only mount, missing
// tmpfs) causes Healthy to return false so the HTTP endpoint honestly
// reports unhealthy. Auto-restart policy is a compose / orchestrator
// concern, not this app's — runProbe deliberately diverges and
// returns exit 0 in degraded mode so that container HEALTHCHECK
// doesn't restart-loop when the operator has not configured
// restart:none.
func (m *healthMarker) Healthy() bool {
