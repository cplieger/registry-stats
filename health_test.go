...
// TestHealthMarker_Healthy covers the Healthy() method introduced in
// Phase-3 step 7 so internal/webapi can consume *healthMarker via the
// api.HealthSignal interface. The method checks marker-file presence
// (the same signal os.Stat(healthFile) used pre-refactor in
// handleHealth) — strict: a missing marker always reports unhealthy,
// regardless of whether the marker dir is writable. Auto-restart
// policy is compose's concern; runProbe deliberately diverges for the
// CLI probe.
...
// Healthy reports false (strict os.Stat semantics): the HTTP
// /api/health endpoint should honestly surface unhealthy when the
// marker directory is not writable. The CLI probe (runProbe)
// deliberately diverges and returns exit 0 in degraded mode so
// containers without restart:none don't loop — that's a separate
// contract tested by TestProbeCheck_degraded.
